/* Young Bin Jo
For this mp, I followed the step given in wiki page. The ultimate goal of this mp was to divide the floorplan vertically and horizontally to 
expand the floor Initially, MP asked to check whether node a and b are leaf, internal or subtree of each other. For leaf, I simply checked 
whether node's left or right is NULL or not. For internal, I just did opposite of leaf. If left or right is not NULL it is internal node. 
For subtree, I used recursion to go through the tree to check whether a is ancestor of b in a tree
For next part, I did rotate. I saved width of the floorplan in temporary variable (i) and saved height in widht and i in height. This switched 
width and height of a floorplan. For recut, I simplied switched the cutline. If it is H it changed to V vice versa. For swap_module I did same
thing as rotate. Simply saved module_a in i and saved module_b in module_a and loaded i in module_b. For swap_topology, I setted 4 conditions
which are a->parents 's right and left and b->parents 's right and left. If it matches, a and b are swaped. For postfix_traversal, I used 
recursion to check right and left of a node and followed the step in get expression for postfix. Lastly for initialize slicing part, I had to
left-skew the floorplan. This means all the right nodes are modules. I initially counted the base case where there is only one node in a tree.
Then this becomes a module not internal node. Therefore I gave NULL for right and left node and UNDEFINED for cutline.
Otherwise, I created another right_node which is a module for ptr struct. I treated all right_node as module and leaf node. Therefore, ptr->right
always became this right_node and for ptr->left I used recursion to repeat the work of creating right_node (module). 
*/
...
int is_leaf_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
	if(ptr -> right == NULL && ptr -> left == NULL)      // if the ptr has a no left/right node attached to it
		return 1;                                        // meaning a module, it should be a leaf node
	else
		return 0;
}
...
int is_internal_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
	if(ptr -> right != NULL || ptr -> left != NULL)	// If the ptr right or left is not NULL it is not leaf_node which means internal node
		return 1;
	else
		return 0;
}
...
int is_in_subtree(node_t* a, node_t* b) {
  // TODO: (remember to modify the return value appropriately)
	...
	if(a == NULL || b == NULL)     // Check whether node a or b are leaf node.
	        return 0;	                  
	if (a == b)			// if node a is node b then one is rooted to other
		return 1;
	if (is_in_subtree(a->right, b) || is_in_subtree(a->left, b)) // else go through the tree to check whether b's ancestor is a or not by recursion
		return 1;
	return 0;
}
...
void rotate(node_t* ptr) {
  // TODO: Rotate the module, swapping the width and height.
	...
	i = ptr -> module -> w;        // Initially store width of module in variable i
	ptr -> module -> w = ptr -> module -> h; //Change width of module to height of module
	ptr -> module -> h = i;	//Change the height of module to variable i which has width of module
...
void recut(node_t* ptr) {
  ...
  assert(ptr->module == NULL && ptr->cutline != UNDEFINED_CUTLINE);
	...
	if(ptr -> cutline == V) 	// If cutline is V change the cutline to H
		ptr -> cutline = H;
    
	else  // If cutline is not V it will be H since undefined_cutline is ignored by assert statement. Then change the cutline to V
		ptr -> cutline = V;
...
// This part works same as rotate.

    module_t *i;

    i = a -> module;	// Store a module in i
    a -> module = b -> module;	// Store b module in a
    b -> module = i;		// Load i module (a module) in b
}
  // TODO:
...
void swap_topology(node_t* a, node_t* b) {
  ...
  // TODO:
	...
	if (a_par -> left == a && b_par -> left == b) {	//If a -> parent's left is a and b -> parent's left is b
		a -> parent = b_par;			//Switch parents
		b -> parent = a_par;	
		a_par -> left = b;			//Switch alphabets (module)
		b_par -> left = a;       
    	}
    
	if (a_par -> right == a && b_par -> left == b) { //If a -> parent's right is a and b -> parent's right is b,
		a -> parent = b_par;			//Switch parents
		b -> parent = a_par;			
		a_par -> right = b;			//Switch alphabets (module)
		b_par -> left = a;			
	}

	if (a_par -> left == a && b_par -> right == b){	//If a -> parent's left is a and b -> parent's right is b
		a -> parent = b_par;			//Switch parents
		b -> parent = a_par;			
		a_par -> left = b;			//Switch alphabets (module)
		b_par -> right = a;			
	...
	if (a_par -> right == a && b_par -> right == b) { //If a -> parent's right is a and b -> parent's right is b
		a -> parent = b_par;			//Switch parents
		b -> parent = a_par;			
		a_par -> right = b;			//Switch alphabets (module)
		b_par -> right = a;			
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  // TODO:
    ...
    expression[*nth].module = ptr->module;   // Use get expression part for the expression
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  // TODO: (remember to remove the following return statement)
	if(n == num_modules - 1) { //If this statement works, it means there is only one node in a tree which could is a root also works as module
...
// Create a leaf node to allocate correct memory for ptr struct
	node_t* right_node = (node_t*)malloc(sizeof(node_t));  
	right_node->module = &modules[n];
	right_node->cutline = UNDEFINED_CUTLINE;	// Since it is leaf node, there is no cutline, and parent is ptr. 
	right_node->parent = ptr;
	right_node->right = NULL;			// right and left of leaf node are NULL
	right_node->left = NULL;
   
	ptr->module = NULL;                             // As in image above, ptr node has no module
	ptr->cutline = V;              			// Cutline is always V to be left_skewed
	ptr->parent = par;				// Parent of ptr is always ptr
	ptr->right = right_node;                        // Right of ptr is right_node
	ptr->left = init_slicing_tree(ptr, n+1);        // Left of ptr should be created again by recalling this function.
	}
